...
from .data_utils import TrainTestPreparation, ZeroInjectionWhereMean, FoldChInjectionWhereMean, DataCookerTrainValidTest
...
class AECorrector(Corrector):
    def __init__(self, param_path=None, denoisingAE=True,
                 inject_zeros=True, epochs=400, encoding_dim=14, lr=0.00098, batch_size=50):
    ...
    def correct(self, counts, size_factors, **kwargs):
        self.loader = DataCookerTrainValidTest(counts, inject_outliers=self.denoisingAE)# size_factors,
        self.data = self.loader.data()
        self.ae = Autoencoder(choose_autoencoder=True,
                              encoding_dim=self.encoding_dim,
                              size=counts.shape[1])
        self.ae.model.compile(optimizer=RMSprop(lr=self.lr), loss=self.ae.loss)
